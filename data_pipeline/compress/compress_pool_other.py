"""Compress non-file_read SEGs with gpt-4.1-mini using system_prompt_other.txt.

Mirrors compress_pool_file_read.py but:
  - default model: gpt-4.1-mini-2025-04-14
  - default prompt: update/system_prompt_other.txt
  - reads `kind` from each entry (no hard-coded file_read)

Usage:
  python update/compress_pool_other.py --limit 50 --label smoke
  python update/compress_pool_other.py --limit 1000 --label test1k
  python update/compress_pool_other.py --limit 10000 --label all10k \
      --existing update/file_read_compressed_test1k_<ts>.jsonl  # to resume

  # Override model (e.g. fall back to gpt-5 if 4.1-mini regresses)
  python update/compress_pool_other.py --limit 1000 --label test1k --model gpt-5
"""
import argparse
import json
import os
...
DEFAULT_PROMPT = ROOT / "system_prompt_other.txt"
DEFAULT_POOL = ROOT / "other_kinds_pool.jsonl"
ENV_PATH = ROOT / ".env"
...
def main():
    ...
    parser.add_argument("--existing", default=None,
                        help="Previous run jsonl. Entries already present (without error) are skipped.")
    parser.add_argument("--retry-errors", action="store_true")
    parser.add_argument("--filter-kind", default=None,
                        help="Only process entries with this kind. Use for targeted re-runs "
                             "(e.g. --filter-kind file_operation after fixing the prompt).")
    ...
    if not Path(args.pool).exists():
        sys.exit(f"Pool missing: {args.pool}. Run extract_other_kinds_pool.py first.")

    system_prompt = prompt_path.read_text(encoding="utf-8")
    print(f"Loaded {prompt_path.name}: {len(system_prompt)} chars")
    ...
    with open(args.pool, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if not line:
                continue
            rec = json.loads(line)
            if args.filter_kind and rec.get("kind") != args.filter_kind:
                continue
            entries.append(rec)
            if len(entries) == args.limit:
                break
    if args.filter_kind:
        print(f"Loaded {len(entries)} entries with kind={args.filter_kind} (limit={args.limit}) from {args.pool}")
    else:
    ...
    if args.existing:
        ...
        if not ex_path.exists():
            sys.exit(f"--existing missing: {ex_path}")
    ...
    print(f"Compressing {len(to_compress)} entries with {args.model} (workers={args.max_workers}) ...")
    t0 = time.time()
    results: dict = {}
    completed = 0
    ...
    out_path = ROOT / f"other_compressed_{args.label}_{timestamp}.jsonl"
    n_drop = 0
    n_error = 0
    n_orphan = 0
    total_prompt = 0
        ...
        for e in entries:
            ...
            if eid in existing_by_id and eid not in results:
                ...
                if ex_rec.get("dropped"):
                    n_drop += 1
                    by_kind_drop[kind] = by_kind_drop.get(kind, 0) + 1
                else:
                    by_kind_ratio.setdefault(kind, []).append(ex_rec.get("ratio", 0.0))
                if ex_rec.get("error"):
                    n_error += 1
            ...
            if body is None:
                ...
                by_kind_ratio.setdefault(kind, []).append(ratio)
    ...
    all_kinds = sorted(set(list(by_kind_ratio) + list(by_kind_drop)))
    for kind in all_kinds:
        arr = by_kind_ratio.get(kind, [])
        n_kept = len(arr)
        n_drop_k = by_kind_drop.get(kind, 0)
        n_total = n_kept + n_drop_k
        avg = sum(arr) / n_kept if n_kept else 0.0
        print(f"  {kind:<22} kept={n_kept}/{n_total} ({100*n_kept/max(1,n_total):.0f}%), "
              f"drop={n_drop_k}, avg ratio (kept)={avg:.3f}")
