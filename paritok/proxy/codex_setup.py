...
def render_codex_config(cfg: CodexConfig, host: str, port: int) -> str:
    ...
    # Only pin a model when explicitly set — otherwise let Codex choose (its
    # -m/--model flag, /model, the panel picker, or its default). A hardcoded
    # `gpt-5` is rejected by ChatGPT-account Codex.
    if cfg.model:
        lines.append(f'model = "{_toml_escape(cfg.model)}"')
    lines += [
